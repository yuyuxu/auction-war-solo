...
var QuizPageHelper = {
  ...
  IsSubmitDataValid : function(submit_data_str) {
    // TEST, uncomment
    // return true;

    submit_data = JSON.parse(submit_data_str);
    var side = $('#player-role').val();
    if (side == "type-0") {
...
$(document).ready(function() {
  ...
  $('#next-stage').click(function() {
    ...
    $('#quiz_data_prev').val(submit_data_str);
    $('#quiz_data_next').val(submit_data_str);
    if (QuizPageHelper.IsSubmitDataValid(submit_data_str)) {
      $('#next-stage').closest('form').submit();
      Logger.Log('quiz: submit next page ' + submit_data_str);
    } else {
      alert('Answer is not correct, please retry.');
    }
  });
});